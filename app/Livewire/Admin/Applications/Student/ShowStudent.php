...
class ShowStudent extends Component
{
    ...
    public function sendFinalAcceptanceLetterPoland()
    {
        try {
            Log::info('=== sendFinalAcceptanceLetterPoland metodu çağırıldı ===');
            Log::info('Student ID: ' . $this->student->id);
            Log::info('Student Email: ' . ($this->student->email ?? 'YOXDUR'));
            ...
            Mail::to($this->student->email)->send(new FinalAcceptanceLetterPolandMail($this->student, $user, $plainPassword));
            ...
            if ($mailDriver === 'log') {
                session()->flash('success', 'Polşa tələbə attestatı log faylına yazıldı. SMTP konfiqurasiyası üçün .env faylında MAIL_MAILER=smtp təyin edin.');
            } else {
                session()->flash('success', 'Polşa tələbə attestatı (EN+PL) ' . $this->student->email . ' ünvanına göndərildi.');
            }
        } catch (\Exception $e) {
            Log::error('Polşa attestatı göndərilərkən xəta: ' . $e->getMessage(), [
                'student_id' => $this->student->id,
                'email' => $this->student->email,
                'trace' => $e->getTraceAsString(),
            ]);
